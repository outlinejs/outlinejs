...
var _gulpGettextParser = require('gulp-gettext-parser');

var _gulpGettextParser2 = _interopRequireDefault(_gulpGettextParser);
...
var _class = function () {
  ...
  _createClass(_class, [{
    ...
    value: function getBrowserify() {
      ...
      var b = this.browserify(Object.assign({}, watch ? _watchify2.default.args : {}, {
    ...
    value: function getBrowserifyBundle(b) {
      ...
      var bundle = b.bundle().on('error', $.util.log.bind($.util, 'Browserify Error')).pipe((0, _vinylSourceStream2.default)(this.getMainFile())).pipe((0, _vinylBuffer2.default)());
      if (clientMode) {
        bundle = bundle.pipe($.sourcemaps.init({ loadMaps: true })).pipe($.sourcemaps.write('./')) // writes .map file
        .pipe(this.gulp.dest('./.tmp/scripts')).on('end', function () {
          reload();
        });
      } else {
    ...
    value: function load() {
      ...
      this.gulp.task('ojs:js-build', ['ojs:node-html', 'ojs:env'], function () {
      ...
      this.gulp.task('ojs:pot', function () {
        return _this2.gulp.src('project/**/*.js').pipe((0, _gulpGettextParser2.default)({})).pipe($.rename('template.pot')).pipe(_this2.gulp.dest('locale/'));
      });
      ...
      this.gulp.task('ojs:images', function () {
      ...
      this.gulp.task('ojs:test', ['ojs:lint-test'], function () {
        var files = _glob2.default.sync('project/**/tests/*.js');
        var b = _this2.browserify({ entries: files }).plugin(_mocaccino2.default, { reporter: 'spec' }).bundle();
      ...
      this.gulp.task('ojs:serve', ['ojs:js-watch', 'ojs:styles', 'ojs:fonts', 'ojs:pot', 'ojs:locale-build'], function () {
