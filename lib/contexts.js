...
var ResponseContext = exports.ResponseContext = function (_DecorableContext) {
  ...
  function ResponseContext(response, request, routeUtils) {
    ...
    console.info('new Response', request);
  ...
  _createClass(ResponseContext, [{
    ...
    value: function navigate(to) {
      ...
      try {
        url = _routers.RouteUtils.reverse(to, params, this.request);
      } catch (ex) {
        url = to;
        if (runtime.isClient) {
      ...
      if (runtime.isClient) {
        if (settings.ROUTING_USE_FRAGMENT) {
          ...
          if (settings.SERVER_SIDE_LINK_ONLY) {
            ...
            _routers.RouteUtils.parseUrl(url);
          }
        }
      } else {
