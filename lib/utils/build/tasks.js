...
var _outlinejsBabelJsxgettext = require('outlinejs-babel-jsxgettext');

var _outlinejsBabelJsxgettext2 = _interopRequireDefault(_outlinejsBabelJsxgettext);
...
var _class = function () {
  ...
  _createClass(_class, [{
    ...
    value: function getBrowserify() {
      ...
      var forNode = arguments.length > 1 && arguments[1] !== undefined ? arguments[1] : false;
      var watch = arguments.length > 2 && arguments[2] !== undefined ? arguments[2] : false;
      var files = arguments.length > 3 && arguments[3] !== undefined ? arguments[3] : [this.projectJsEntry];
    ...
    value: function getBrowserifyBundle(b) {
      var clientMode = arguments.length > 1 && arguments[1] !== undefined ? arguments[1] : true;

      var bundle = b.bundle().on('error', $.util.log.bind($.util, 'Browserify Error')).pipe((0, _vinylSourceStream2.default)(this.getMainFile())).pipe((0, _vinylBuffer2.default)()).pipe($.sourcemaps.init({ loadMaps: true })).pipe($.uglify()).pipe($.sourcemaps.write('./')); // writes .map file
      if (clientMode) {
    ...
    value: function load() {
      ...
      this.gulp.task('ojs:js-watch', ['ojs:node-html', 'ojs:env', 'ojs:locale-build'], function () {
      ...
      this.gulp.task('ojs:styles', function () {
        return _this2.gulp.src('project/**/styles/*.scss').pipe($.plumber()).pipe($.sourcemaps.init()).pipe($.sass.sync({
          ...
          precision: 10,
          includePaths: ['.']
        }).on('error', $.sass.logError)).pipe($.postcss([require('autoprefixer')({ browsers: ['> 1%', 'last 2 versions', 'Firefox ESR'] })])).pipe($.concat('main.css')).pipe($.cssnano()).pipe($.sourcemaps.write('./')).pipe(_this2.gulp.dest('.tmp/styles')).pipe(reload({ stream: true }));
      });
      ...
      this.gulp.task('ojs:html', ['ojs:js-build', 'ojs:styles'], function () {
        return _this2.gulp.src('project/*.html').pipe($.useref({ searchPath: ['.tmp', '.'] })).pipe($.if('*.html', $.htmlmin({ collapseWhitespace: true }))).pipe(_this2.gulp.dest('dist'));
      });

      this.gulp.task('ojs:node-app', ['ojs:js-build'], function () {
        return _this2.gulp.src('.tmp/node-scripts/*.js').pipe(_this2.gulp.dest('dist/node-scripts'));
      });
