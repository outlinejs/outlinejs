  ...
  _createClass(_class, [{
    ...
    value: function load() {
      ...
      this.gulp.task('ojs:html', ['ojs:js-build', 'ojs:styles'], function () {
        return _this2.gulp.src('project/*.html').pipe($.useref({ searchPath: ['.tmp', '.'] })).pipe($.if('*.js', $.uglify())).pipe($.if('*.html', $.htmlmin({ collapseWhitespace: true }))).pipe(_this2.gulp.dest('dist'));
      });
